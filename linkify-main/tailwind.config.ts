...
function addVariablesForColors({ addBase, theme }: any) {
  ...
  let newVars = Object.fromEntries(
    Object.entries(allColors)
      // semantic tokens are defined as hsl(var(--x)) — emitting them here would
      // create circular var definitions that invalidate the globals.css values
      .filter(([, val]) => !String(val).includes("var(--"))
      .map(([key, val]) => [`--${key}`, val])
  );
