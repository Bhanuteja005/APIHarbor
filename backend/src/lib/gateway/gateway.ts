...
const createQuicConnection = async (
  ...
  // Lazily import the native QUIC bindings so that merely importing this module
  // doesn't crash on platforms without a prebuilt binary (e.g. Windows). The
  // gateway/relay feature is the only consumer and only needs QUIC at call time.
  const quicModule = await import("@infisical/quic");
  const quic = quicModule.default || quicModule;
