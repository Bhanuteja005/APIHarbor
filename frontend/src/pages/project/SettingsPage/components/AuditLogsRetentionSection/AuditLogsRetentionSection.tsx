...
export const AuditLogsRetentionSection = () => {
  ...
  const handleAuditLogsRetentionSubmit = async ({ auditLogsRetentionDays }: TForm) => {
    if (!subscription?.auditLogs) {
      handlePopUpOpen("upgradePlan", {
        text: "Configuring audit logs retention can be unlocked if you upgrade to APIHarbor Pro plan."
      });
    ...
    if (subscription && auditLogsRetentionDays > subscription?.auditLogsRetentionDays) {
      handlePopUpOpen("upgradePlan", {
        text: "Updating audit logs retention period to a higher value can be unlocked if you upgrade to APIHarbor Pro plan."
      });
  ...
  // render only for dedicated/self-hosted instances of APIHarbor
  if (
