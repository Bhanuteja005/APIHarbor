...
export const SnapshotView = ({
  ...
  const { mutateAsync: performRollback, isPending: isRollingBack } = usePerformSecretRollback();

  const { data: snapshotData, isPending: isSnapshotLoading } = useGetSnapshotSecrets({
    snapshotId,
    env: environment
  });
    ...
    <>
      ...
      <NoticeBannerV2 title="Snapshots are being deprecated" className="my-2">
        <p className="my-1 text-sm text-mineshaft-300">
          ...
          <a
            target="_blank"
            href="https://apiharbor.com/docs/documentation/platform/pit-recovery"
